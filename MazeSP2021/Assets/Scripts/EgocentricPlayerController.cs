    ...
    IEnumerator MovetoIntersection(System.Action<Vector3> callback)
    {
        ...
        else if (direction == right)
        {
            ...
            for (int col = playerPosition[1] + 1; col < mazeArray.GetLength(1); col++)
            {
                ...
                // Intersection, End, Start
                else if (mazeArray[playerPosition[0], col] == "0I" || mazeArray[playerPosition[0], col][1] == 'F' || mazeArray[playerPosition[0], col] == "0S")
                {
                    if (mazeArray[playerPosition[0], col][1] == 'F' && PersistentManager.Instance.audioWaitActive)
                    {
                        moveVector.z += 1;
                        playerPosition[1] = col;
                        break;
